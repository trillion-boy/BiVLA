...
>
> 뒤의 두 측정은 **stack 하나만 다르고 나머지는 완전히 동일**하다. 설정
> 차이가 아니라 stack이 경계선 태스크라 재실행마다 흔들리는 것으로 본다 —
> `do_sample=False`(greedy)임에도 GPU 커널 비결정성으로 아슬아슬한
> 에피소드가 뒤집힌다. 같은 현상이 SpatialVLA에도 기록돼 있다:
> stack baseline을 두 번 돌려 33.3%와 29.2%가 나왔고 grasp는 두 번 다
> 58.3%였다(`docs/VISUAL_TOKENS_VS_LATENCY.md`).
>
> 따라서 **각 캠페인의 Δ는 그 캠페인 자신의 baseline 기준으로만 계산한다.**
> 이 절의 Δ는 78.1%, depth pruning의 Δ는 74.0% 기준이며, 두 캠페인의 Δ를
> 같은 표에 나란히 놓고 절대 성공률로 비교해서는 안 된다.
>
> 앞으로의 실행은 **개입 조건과 baseline을 같은 세션에서 연속으로 돌리고
> 두 JSON을 모두 레포에 커밋한다.** 위 두 캠페인이 JSON을 남기지 않아
> 산문 속 숫자만 남았고, 그래서 어느 쪽이 맞는지 사후 검증이 불가능하다.
