...
- **foveation은 4개 태스크 전부 개선**, log-polar(+18.8pp)와
  blur(+17.7pp)가 **평균으로는 거의 동률**. 둘 다 짝지은 검정을
  통과한다(McNemar exact p=0.0051 / 0.0060).

> **정정 (2026-08-05): "변형 무관"은 평균에서만 참이다.**
> 태스크별로 보면 두 변형은 거의 상보적이다 — blur는 eggplant에서
> 압도적(62.5 vs 33.3)이고, log-polar는 stack(45.8 vs 20.8)과
> spoon(41.7 vs 25.0)에서 압도적이다. **평균이 같다고 같은 개입이
> 아니다.** 아래 "변형 무관하게 확실한 이득"이라는 결론은 집계
> 수준에서만 유효하며, 태스크 단위 분석은
> `experiments/OpenVLA_Bridge_Grid.md` §3 참조.
...
> **UniVLA/Bridge baseline은 캠페인마다 별도로 측정됐다 — 섞어 쓰면 안 된다.**
> 같은 체크포인트·같은 4 task·같은 N=24인데 세 번의 측정이 서로 다르다:
>
> | 측정 | 시점 | eggplant | carrot | stack | spoon | 평균 | 원본 JSON |
> |---|---|---|---|---|---|---|---|
> | 레퍼런스 (멘토님 기록과 일치) | 2026-06-18 | 24/24 | 17/24 | 18/24 | 20/24 | 82.3% | `results/vanilla_baselines/univla_full_suite/` |
> | depth pruning 캠페인 | 2026-06-29 | 24/24 | 16/24 | **14/24** | 17/24 | 74.0% | 미보관 |
> | **이 절 (chunk-exec / foveation)** | 2026-07-20 | 24/24 | 16/24 | **18/24** | 17/24 | **78.1%** | 미보관 |
>
> **2026-08-05 재측정으로 결론이 났다: 78.1%가 맞다.** action-repeat 실험과
> 같은 세션에서 baseline을 다시 돌렸더니 **4개 태스크 전부** 2026-07-20
> 캠페인과 개수까지 일치했다(24/16/18/17 = 75/96 = 78.1%). depth pruning
> 캠페인의 stack 14/24는 재현되지 않는다.
>
> | task | 07-20 | **08-05 재측정** | 06-29 (depth) |
> |---|---|---|---|
...
> | 평균 | 78.1% | **78.1%** ✓ | 74.0% |
>
> depth 캠페인이 무엇이 달랐는지는 이제 알 수 없다 — per-episode JSON을
> 남기지 않았기 때문이다. 그 캠페인의 Δ는 그 캠페인의 74.0% 기준으로
> 읽되, **그 숫자를 다른 표로 옮기지 않는다.** (개수 일치가 에피소드
> 일치를 뜻하지는 않는다. 07-20 쪽도 per-episode 기록이 없어서 그
> 수준까지는 확인할 수 없다. 다만 4/4 일치면 판단 근거로 충분하다.)
>
> 앞으로의 실행은 **개입 조건과 baseline을 같은 세션에서 연속으로 돌리고
> 두 JSON을 모두 레포에 커밋한다.** 위 두 캠페인이 JSON을 남기지 않아
> 이 문제가 두 달간 잡히지 않았다. 짝지어진 JSON이 있으면
> `adaptive_sparse_vla/paired_test.py`로 McNemar 검정까지 바로 된다.
