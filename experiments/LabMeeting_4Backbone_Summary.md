...
> ⚠️ **정정 (Stack baseline).** 이 표는 원래 Stack을 75.0%로 기록하고
> 전체 baseline을 78.1%로 잡았으나, **맞는 값은 58.3%이며 따라서 전체
> baseline은 73.9%**입니다(`docs/DEPTH_PRUNING_RESULTS.md`의 base 행과
> 일치). 아래 Δ 값들은 **모두 78.1% 기준으로 계산된 옛 값**이므로
> 73.9% 기준으로 다시 계산해야 합니다. 영향 범위는 이 절 아래
> **UniVLA/Bridge의 모든 Δ**입니다.
>
> | 조건 | 절대 성공률 | 옛 Δ (vs 78.1) | **새 Δ (vs 73.9)** |
> |---|---|---|---|
> | chunk-exec k=2 | 65.6% | −12.5 | **−8.3** |
> | foveate log-polar 20% | 86.5% | +8.3 | **+12.6** |
> | foveate blur 20% | 76.0% | −2.1 | **+2.2** ← 부호 반전 |
>
> blur가 **손해에서 이득으로 부호가 바뀝니다**(둘 다 노이즈 범위이지만
> 표에 적히는 부호가 달라집니다). 조건별 per-task 수치 자체는 그대로이고
> 바뀌는 것은 기준선뿐입니다.
