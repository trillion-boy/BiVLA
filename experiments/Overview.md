...
> | 문서 | 무엇 | 분량 |
> |---|---|---|
> | **이 문서** | **연구 전체를 10분에** | ~470줄 |
> | `Report.md` | 어떻게 쟀고(§3) 무엇이 나왔고(§4–6) 무엇이 남았나(§7) | ~2,300줄 |
> | `RelatedWork.md` | 선행연구와 논문별 노트 | ~1,240줄 |
...
VLA 정책은 느리다 — 관측 하나에 2.8초가 걸리는 모델도 있다(UniVLA, L4). 그래서 **학습 없이
...
여덟 칸이다 — 격자에서 **38번**을 쟀으므로(이 표의 35칸 + `prune 8` 두 칸 +
`prune 2 + repeat 2` 한 칸), 우연히 나올 수 있는 것을 걸러내려면
p < 0.0013이라야 인정한다.
...
> **38번 중 여덟 번만 통과한다.** 나머지는 방향이 보여도 그 칸 하나로는
...
2. **같은 행 안에서 82포인트가 벌어진다.** action repeat 2는 SpatialVLA/Bridge
...
### ① 같은 방법인데 어느 층을 지우느냐로 45.9포인트가 갈린다
...
pick 태스크 셋의 성공이 전부 0/25가 된다.

**옮긴 것은 "후보 구간"과 선택 규칙이다.** 층 지우기는 두 단계다 — ① 지워도
되는 구간(창)을 정하고, ② 그 안에서 **가장 있으나 마나 한 층** 순으로 4개를
고른다. 위 네 줄 중 **셋은 ①(창)을 옮긴 것**이고, 하나([17,23,27,31])는 창을
기본 그대로 두고 **②에 간격 규칙만 더한 것**이다(고른 층끼리 3층 이상
떨어지게).

| | 창 | 후보 층 수 | |
|---|---|---:|---|
| **기본 창** — 1행·3행 | L16–31 | 16개 | 뒷**절반**에서 고른다 (3행은 +간격 3 규칙) |
| **앞으로 넓힌 창** — 2행 | L2–31 | 30개 | 앞쪽 층까지 후보 |
| **뒤로 옮긴 창** — 4행 | L28–31 | **4개** | **맨 끝 4층**만 후보 |

**뒤로 옮긴 창은 후보가 4개인데 4개를 지운다.** 고를 여지가 없어 그 순위가
아무 일도 하지 않고 마지막 4층이 강제로 지워진다 — −30.4는 "BI가 고른 가장 나쁜 조합"이
아니라 **선택 자체가 없는 조건**의 값이다.

**그러므로 논문에 "층 4개를 지웠다"고만 적으면 그 실험은 재현되지 않는다.**
위 네 줄이 전부 **같은 순위 기준(BI), 같은 개수(4층)**인데 결과가 +15.6에서
−30.4까지 간다. 결과를 가른 것은 방법의 성능이 아니라 **구현이 후보 구간을
어디에 뒀고 선택에 무슨 제약을 걸었는가**이다. 그리고 이 값은 **선행 논문들이
보고하지 않는 것이 아니라, 서로 다르게 처방한다.** ShortGPT와 EfficientVLA는
전 층을 훑고 제약을 걸지 않는데, Gromov et al.(ICLR 2025)은 깊은 연속 블록을
지우면서 *"마지막 층을 남기는 것이 필수"*라고 보고한다. 둘을 로봇 정책에서
비교한 사람은 없다. (원문 확인: `paper/TableI_Cells.md`. "보고하지 않는다"고
쓰면 틀린다.)
...
결론 낸 논문이, 벤치마크만 바꾸면 45.9포인트로 흔들릴 수 있다. 즉 **층 선택
...
> 되돌리므로 **출력의 모양은 안 변한다** — 정중앙에 표식을 찍어 왕복시키면
> **0.00 px** 자리에 그대로 돌아온다(가장자리도 1 px 이내). 당근이나 접시가
> 늘어나 보이지 않는 이유다.
...
**백본만 바꾼 경우** — 벤치마크를 고정하고 백본만 바꾼다. Bridge는 세 백본이
다 돌아 세 쌍, Fractal은 두 백본이라 한 쌍이다.
...
백본을 바꾸면 **여섯**이고 그중 다섯이 p < 10⁻⁴다. 세 백본이 모두 참여하고,
**두 벤치마크 모두에서** 통과한다.

**둘째 줄이 가장 깨끗한 부호 뒤집힘이다.** 같은 Fractal, 같은 135 에피소드,
같은 `depth prune 4`인데 OpenVLA는 **+15.6**, SpatialVLA는 **−17.8**이다 —
**양쪽이 각자 기준을 통과하면서 부호가 반대**인 유일한 쌍이다.

**차이가 가장 큰 행은 action repeat 2다.** 같은 벤치마크, 같은 개입, 같은 k인데
SpatialVLA는 **오르고**(+12.5) UniVLA는 **크게 떨어진다**(−69.8). 82포인트
...
> **왜 UniVLA만 크게 떨어지는지는 우리가 모른다.** 관찰만 적는다 — UniVLA는 행동을
> 이산 토큰으로 내놓고, 한 번 예측한 것을 두 스텝 실행하는 게 이 백본에서
> 유독 손실이 크다. 토크나이저 탓인지 학습 분포 탓인지 우리 데이터는 구별하지
> 못한다.
...
> 들어가고, Fractal의 백본 축은 쌍이 하나뿐이다. 그래서 **벤치마크 축은 백본
> 둘에, 백본 축은 Bridge 쪽에** 무게가 쏠려 있다. 세 번째 벤치마크가 필요한
> 이유가 이것이다.
...
않는다. 그래서 하락이 가장 컸던 칸(SpatialVLA/Fractal, depth prune 4)에서
실패를 **종류별로** 세봤다.
...
| 검증 | 문서의 숫자를 전부 기록에서 재계산해 대조 — **일치. 대조 과정에서 나온 불일치는 98건이고 전부 고쳤다**(`Report.md` §7.1) |
| baseline | 다섯 칸 전부 원논문 값과 대조 — **넷이 높고 하나가 4.2포인트 낮다**(SpatialVLA/Bridge 30.2% 대 34.4%). 설정이 틀렸다면 *일관되게* 낮아야 하므로 논지는 그대로다 |
| 선행연구 | **10편** 원문 PDF 대조 완료 — 선행연구 7편 + 백본 3편 |
...
|---|---|---|---|
| 분량 | ~470줄 | ~2,300줄 | ~1,240줄 |
| 목적 | **10분에 파악** | 근거·검증·기록 | 남이 잰 것 |
| 안에 있는 것 | 결과 셋과 현황 | 모든 표, 정정 기록 98건, 기계 검증 | §2 선행연구 + 논문별 노트 7편 |
