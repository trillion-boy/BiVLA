...
> | `Report.md` | 어떻게 쟀고(§3) 무엇이 나왔고(§4–6) 무엇이 남았나(§7) | ~2,255줄 |
...
여덟 칸이다 — 격자에서 **35번**을 쟀으므로, 우연히 나올 수 있는 것을 걸러내려면
p < 0.0014라야 인정한다.
...
| 검증 | 문서의 숫자를 전부 기록에서 재계산해 대조 — **일치. 대조 과정에서 나온 불일치는 86건이고 전부 고쳤다**(`Report.md` §7.1) |
...
|---|---|---|---|
| 분량 | ~460줄 | ~2,255줄 | ~1,240줄 |
| 목적 | **10분에 파악** | 근거·검증·기록 | 남이 잰 것 |
| 안에 있는 것 | 결과 셋과 현황 | 모든 표, 정정 기록 86건, 기계 검증 | §2 선행연구 + 논문별 노트 7편 |
