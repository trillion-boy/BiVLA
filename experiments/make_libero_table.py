...
def main():
    data = load()
    # emit in the resizebox layout: one backbone triple per line, blank line
    # between rows, a section comment and rotated label per suite.
    fam_order = [lbl for _, lbl in FAMILIES]
    blocks = []
    for env, label, _ in SUITES:
        lines = [f"% =========================================================",
                 f"% {label.upper()}",
                 f"% =========================================================",
                 f"\\multirow[c]{{6}}{{*}}{{\\rotatebox[origin=c]{{90}}{{{label}}}}}"]
        for fi, (fam, fname) in enumerate(FAMILIES):
            trip = []
            for _, _, bk in BACKBONES:
                row = data.get((bk, env, fam))
                bas = data.get((bk, env, "original"))
                if row is None or bas is None:
                    trip.append("-- & -- & --"); continue
                s3, l3, n3 = metrics(row)
                bs3, bl3, bn3 = metrics(bas) if fam != "original" else (None,) * 3
                if bk == "smolvla" and fam in ("guarded_reuse", "temporal_fusion"):
                    l3, bl3 = None, None
                trip.append(" & ".join([cell(s3, bs3, True), cell(l3, bl3, False),
                                        cell(n3, bn3, False)]))
            lines.append(f"& {fname}")
            lines.append(f"& {trip[0]}")
            lines.append(f"& {trip[1]}")
            lines.append(f"& {trip[2]} \\\\")
            lines.append("")
        blocks.append("\n".join(lines).rstrip())
    body2 = "\n\n\\cmidrule(lr){1-11}\n\n".join(blocks)
    heads2 = "\n".join(
        f"& \\multicolumn{{3}}{{c}}{{{n} ({p})}}" for n, p, _ in BACKBONES)
    sub2 = ("& & Success $\\uparrow$ (\\%) & Latency $\\downarrow$ (ms) & "
            "Avg. Steps $\\downarrow$\n" +
            "\n".join(["& Success $\\uparrow$ (\\%) & Latency $\\downarrow$ (ms) & "
                        "Avg. Steps $\\downarrow$"] * (len(BACKBONES) - 1)))
    tex = f"""%% ---------------------------------------------------------------------------
...
%% Parameter counts as reported by the original works. SmolVLA guarded reuse
%% and temporal fusion ran under a second implementation on other GPUs, so
%% their latency is printed as "--". Needs booktabs, multirow, graphicx,
%% xcolor. Spans both columns.
%% ---------------------------------------------------------------------------
\\begin{{table*}}[t]
\\centering
...
\\toprule
\\multirow{{2}}{{*}}{{Task}} & \\multirow{{2}}{{*}}{{Policy}}
{heads2} \\\\
\\cmidrule(lr){{3-5}}
\\cmidrule(lr){{6-8}}
\\cmidrule(lr){{9-11}}
