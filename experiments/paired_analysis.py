...
Outputs experiments/paper/paired_results.csv (one row per table cell) and
experiments/paper/PairedResults.md (readable tables), plus the full sweep
over every configuration the mentor ran, not only the one per family that
the tables show: experiments/paper/paired_results_all.csv and
experiments/paper/PairedResultsAll.md (one row per backbone, environment or
suite, and configuration, with an in_table flag). The script ends by
checking that every episodes.jsonl under the results folder was read and
stops with an error if any was not. No scipy is needed.
"""
import csv
import glob
import json
...
def episodes(run_dir, suite=None):
    ...
    for f in glob.glob(pattern, recursive=True):
        FILES_READ.add(os.path.abspath(f))
        for line in open(f):
            if not line.strip():
                continue
            d = json.loads(line)
...
def main():
    ...
    # Full sweep: every configuration the mentor ran, paired with Original.
    in_table = {(r["backbone"], r["env"], r["configuration"]) for r in rows}
    all_rows = []
    for env, dirs in SIMPLER.items():
        for bk, rd in dirs.items():
            orig = episodes(os.path.join(RES, rd, "original"))
            for cfg in sorted(os.listdir(os.path.join(RES, rd))):
                if cfg == "original" or not os.path.isdir(os.path.join(RES, rd, cfg)):
                    continue
                fam = next(p for p in FAMILIES if cfg.startswith(p))
                var = episodes(os.path.join(RES, rd, cfg))
                r = compare(orig, var)
                all_rows.append(dict(backbone=NAME[bk], env=ENV_LABEL[env],
                                     family=LABEL[fam], configuration=cfg,
                                     in_table=int((NAME[bk], ENV_LABEL[env], cfg) in in_table),
                                     **r))
    for bk, rd in LIBERO_DIR.items():
        for env, suite in LIBERO_SUITE.items():
            orig = episodes(os.path.join(RES, rd, "original"), suite)
    ...
    every = {os.path.abspath(f) for f in
             glob.glob(os.path.join(RES, "**", "episodes.jsonl"), recursive=True)}
    unread = sorted(every - FILES_READ)
    if unread:
        raise SystemExit(f"{len(unread)} episodes.jsonl files were not read, "
                         f"first: {unread[:3]}")
    print(f"coverage: all {len(every)} episodes.jsonl files read")

    fields = ["backbone", "env", "family", "configuration", "n", "win", "loss",
              "delta_pts", "p_mcnemar", "dsteps", "dsteps_lo", "dsteps_hi",
              "dtime_s", "unpaired"]
    with open(os.path.join(HERE, "paper", "paired_results_all.csv"), "w",
              newline="") as fh:
        w = csv.DictWriter(fh, fieldnames=fields + ["in_table"])
        w.writeheader()
        for r in all_rows:
            w.writerow({k: (f"{r[k]:.4f}" if isinstance(r[k], float) else r[k])
                        for k in fields + ["in_table"]})
    md_all = ["# Paired comparison of every configuration with Original",
              "",
              "Generated by experiments/paired_analysis.py. Same columns as "
              "PairedResults.md, but one row per configuration the mentor ran "
              "(13 per backbone and environment or suite), not only the one "
              "the tables show; the table rows are marked with * after the "
              "setting name.", ""]
    envs_all = []
    for r in all_rows:
        if r["env"] not in envs_all:
            envs_all.append(r["env"])
    for env in envs_all:
        md_all += [f"## {env}", "",
        ...
        for r in all_rows:
            ...
            star = "*" if r["in_table"] else ""
            md_all.append(
                f"| {r['backbone']} | {r['family']} | {r['configuration']}{star} | "
                f"{r['n']} | {r['win']} | {r['loss']} | {r['delta_pts']:+.1f} | "
                f"{flag}{r['p_mcnemar']:.3f}{flag} | "
                f"{r['dsteps']:+.1f} [{r['dsteps_lo']:+.1f}, {r['dsteps_hi']:+.1f}] | "
                f"{r['dtime_s']:+.2f} |")
        md_all.append("")
    sig_all = [r for r in all_rows if r["p_mcnemar"] < 0.05]
    md_all += [f"## Configurations with p < 0.05 ({len(sig_all)} of {len(all_rows)})", ""]
    for r in sorted(sig_all, key=lambda r: r["p_mcnemar"]):
        md_all.append(f"- {r['backbone']} / {r['env']} / {r['configuration']}"
                      f"{' (in table)' if r['in_table'] else ''}: "
                      f"{r['delta_pts']:+.1f} pts, win {r['win']} loss {r['loss']}, "
                      f"p = {r['p_mcnemar']:.4f}")
    md_all.append("")
    with open(os.path.join(HERE, "paper", "PairedResultsAll.md"), "w") as fh:
        fh.write("\n".join(md_all))
    print(f"wrote paired_results_all.csv ({len(all_rows)} configurations) and "
          f"PairedResultsAll.md; {len(sig_all)} with p < 0.05")
