...
the one summary.csv selected for the table. Episodes are paired by
...
def selected(csv_path):
    """{(model key, environment, family): configuration} from a summary.csv"""
    out = {}
    for r in csv.DictReader(open(csv_path)):
        bk = next(k for k in NAME if k in r["model_name"])
        fam = next(p for p in FAMILIES if r["configuration"].startswith(p))
        out[(bk, r["environment"], fam)] = r["configuration"]
    return out
...
def main():
    ...
    for env, dirs in SIMPLER.items():
        sel = selected(os.path.join(SUMM, env, "summary.csv"))
        for bk, rd in dirs.items():
            orig_cfg = sel[(bk, next(e for (b, e, f) in sel if b == bk), "original")]
            orig = episodes(os.path.join(RES, rd, orig_cfg))
            for fam in FAMILIES[1:]:
                cfg = next((c for (b, e, f), c in sel.items()
                            if b == bk and f == fam), None)
                if cfg is None:
                    continue
    ...
    sel = selected(os.path.join(SUMM, "libero", "summary.csv"))
    ...
    md = ["# Paired per-episode comparison with the Original policy",
          ...
          "II cell: the variant summary.csv selected for that family, paired "
