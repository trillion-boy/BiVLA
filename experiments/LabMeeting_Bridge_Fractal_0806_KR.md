...
| **계산** | 26개 레이어 중 4개 (15%) | **−17.8** | 0.0002 | 호출당 1.17× |
| **시각** | 관측 정보의 80%, log-polar | +0.7 | 1.00 | 없음 |
| **시각** | 관측 정보의 80%, blur | −1.5 | 0.83 | 없음 |

**시각의 5분의 4는 공짜다. 계산의 15%는 18점이다. 재계획의 4분의 3은 40점이다.**
칸 **내부에서는** 아무 단서가 안 붙는다 — 정책 하나, 벤치마크 하나, 세션 하나,
전부 프로토콜 전체에 대한 paired 비교다.
...
축으로 바뀐다. 반대로 OpenVLA/Bridge에서는 depth prune이 평탄한 축이다
(+1.0, p=1.00). 즉 `시간 ≫ 계산 ≫ 시각`은 **SpatialVLA/Fractal의 결과**이지
VLA 정책 일반에 대한 발견이 아니다.
...
| **SpatialVLA** depth prune 4 | legacy (prune 1만) | **완료** |
...
1. **OpenVLA / Fractal blur.** Bridge 열에서는 log-polar과 blur가 1점 안쪽으로
   일치한다(+18.8 / +17.7). Fractal blur도 −19 근처로 나오면 손실은 **얼마나
   지웠는가**의 문제고, 평탄하게 나오면 **log-polar 특유의 기하**, 즉 주변시의
   문제다. 실행 하나로 메커니즘 둘이 갈린다. 남은 것 중 제일 싸다.
2. SpatialVLA / Bridge foveation을 에피소드 기록과 함께 재측정 — legacy 두 칸을
   paired test로 바꾼다. baseline이 이미 디스크에 있어 추가 비용이 없고,
   repeat 행처럼 foveation 행도 완성된다.
3. baseline을 같은 세션에서 한 번 더 돌려 **에피소드별 노이즈 바닥**을 측정.
   지금 이 보고서의 모든 Δ는 알려지지 않은 바닥 위에서 읽히고 있다. SpatialVLA
   log-polar은 135개 중 15개를 뒤집으면서 순증이 0인데, 그 15개 중 몇 개가
   개입 탓인지 아직 말할 수 없다.
4. log-polar에서 `move_near_v0` 실패 장면을 직접 볼 것. 한 태스크에서 −41.7,
