...
import imageio_ffmpeg, subprocess, json, os, glob
...
def enc(env,model,cfg,task,k,speed,out):
    ...
    subprocess.run([ff,'-y','-loglevel','error','-i',src,'-vf',f'setpts=PTS/{speed},scale={scale}','-r','10','-an','-c:v','libx264','-pix_fmt','yuv420p','-profile:v','baseline','-level','3.0','-crf','26','-movflags','+faststart',out],check=True)
