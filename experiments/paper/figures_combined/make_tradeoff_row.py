...
def tradeoff_row(name, env='bridge', height=2.5):
    ...
    def h(i):
        ...
        return Line2D([], [], marker=markers[i], ls='', color=common.COLORS[f], markersize=4, label=labels[f][j])
    original = Line2D([], [], marker='*', color='#202020', ls='', markersize=7, label='Original')
    # One legend, two rows, read left to right with a trick's settings side by side.
    # Matplotlib fills legend columns top to bottom, so the two rows are interleaved.
    row1 = [original] + [h(i) for i in range(1, 8)]      # foveation, action repeat, depth pruning
    row2 = [h(i) for i in range(8, 14)]                   # guarded reuse, temporal fusion
    blank = lambda: Line2D([], [], ls='', marker='', label=' ')
    row2 += [blank() for _ in range(len(row1) - len(row2))]
    handles = [x for pair in zip(row1, row2) for x in pair]
    fig.legend(handles=handles, loc='outside lower center', ncol=len(row1), frameon=False, handletextpad=.3, columnspacing=1.0)
