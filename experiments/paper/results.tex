...
which the seeds of Section~V should test. Two tricks never lower success significantly in
their best cell on any of the seven backbones, guarded reuse and temporal
fusion, and guarded reuse does so in none of its 66 settings, although
...
speedup and its success change. The favorable corner, faster and no
worse, is empty on SpatialVLA and UniVLA and holds at most four settings
elsewhere, none more than a few points above the original except depth
pruning on CogACT. Action repeat is the only trick
...
layer is faster than the original beyond the latency floor on 14 of the
22 pairs and costs success
...
OpenVLA LIBERO and about 5 percent or less elsewhere, and is the original policy
where they stay shut. Foveation makes every backbone slower, by 3 to 17
percent on six of them and by under 4 percent on UniVLA, whose blur is
amortized over a chunk, and task-aware fusion adds 7 to 17 percent to
every call on OpenVLA, MiniVLA and UniVLA, since the blur and the
attention collection add time without removing any computation. \textbf{[A-2] Action repeat
...
guarded reuse table cells fired on at most five steps and match the original on every
episode but one, and conservative-adaptive fusion reused no patch on
most steps of SpatialVLA and UniVLA. We therefore report how often each gate
