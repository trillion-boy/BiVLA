...
\title{\LARGE \bf
Bag of Tricks for  Training-Free VLA Models from a Visual Representation Perspective
}
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{figure/overview_A3.pdf}
  ...
  with the unmodified policy on matched episodes.}
  \label{fig:overview}
\end{figure*}
...
Fig.~\ref{fig:overview} marks where each intervention enters the control loop.
% This unified
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{figure/success_change_bridge_row.pdf}
...
\begin{figure*}[!t]
  ...
  \caption{Depth pruning and action repeat across every backbone and
  ...
  factor. Success change is from dense in percentage points, and speedup is
  dense time per step divided by intervened time per step. LIBERO suites are pooled per backbone, so 13
...
Figs.~\ref{fig:success-widowx} to~\ref{fig:ablation-depth} show every
...
(Fig.~\ref{fig:ablation-depth}, right). Depth pruning lowers latency per call at
