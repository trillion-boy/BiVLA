...
| 34 | ShortGPT ranks layers by Block Influence, one minus the mean cosine between a layer's input and output, and constrains nothing further | **PDF read 2026-09-04**, Eq. 1: *"BI_i = 1 − E_{X,t} X_{i,t}^T X_{i+1,t} / (‖X_{i,t}‖ ‖X_{i+1,t}‖)"*; removal is by ascending BI. Searched the full text for "adjacent", "contiguous", "consecutive": zero hits, so no adjacency or protected-region constraint. |
| 35 | SmolVLA keeps only part of the VLM's layers by design | **PDF read 2026-09-04**: *"We use only the first 16 layers of the large language model (LLM) within the VLM"*; Fig. 1 caption: *"discarding the last L − N layers"*. |
| 36 | vla-eval unifies fourteen benchmarks and documents pitfalls earlier work left unrecorded | **PDF read 2026-09-04**, abstract: *"The framework supports 14 simulation benchmarks and six model servers"*, *"documenting previously undocumented pitfalls"*; contributions: *"a single undocumented parameter shifts success rates by up to 55 percentage points"*. |
| 37 | TurboVLA drops the language-model pathway rather than removing layers | **PDF read 2026-09-04**, abstract: *"reformulates the conventional V → L → A pathway as a direct V + L → A mapping. Instead of using a large language model as the central interface ... independently encodes visual observations and language instructions ... and predicts continuous action chunks with a compact decoder"*; 0.2B parameters. The earlier wording "build such reductions in by design" (layer removal) was wrong for this paper. |
| 38 | Gaze-Reg is a training-time regularizer with no gaze at inference; it runs pi0 on LIBERO and ALOHA-Sim and OpenVLA on LIBERO only, leaving the OpenVLA x ALOHA cell empty | **PDF read 2026-09-04** (CVPRW volume, paper at pages 1-8), abstract: *"aligns VLA models' internal attention with human visual patterns without architectural modifications or inference-time overhead"*; Sec. 4: *"all models operate without gaze input during inference"*; *"We employ the LIBERO suite for comprehensive in-domain analysis, ALOHA-Sim for cross-domain generalization, and OpenVLA for architectural transferability"*; Table 3 is OpenVLA on LIBERO only. The earlier wording "policies conditioned on gaze" was wrong for this paper. |
| 39 | Gromov et al. keep the final layer and remove a contiguous deep block | **PDF read 2026-09-04**: *"we identify the optimal block of layers to prune by considering similarity across layers"*; *"deeper layers are qualitatively more similar to neighboring layers than shallow layers (with the exception of the very final layer). This suggests an even simpler pruning strategy: remove layers beginning at the penultimate layer"*. They also argue non-contiguous pruning "shouldn't work as well". |
| 40 | VLA-Cache mechanism | **PDF read 2026-09-04**, abstract: *"identifies minimally changed tokens between adjacent frames and reuses their cached key-value representations ... selectively re-computes task-relevant tokens"*; *"filters out semantically important ones based on attention scores from the VLA decoder"*. The earlier clause "so history is retained unlike under token dropping" has no counterpart in the paper (zero hits for "history", "historical") and was removed. |
| 41 | VLA-Cache attributes the poor transfer of VLM token pruning to ignoring temporal structure | **PDF read 2026-09-04**, Sec. 2: *"These intra-frame strategies reduce redundancy within a single image but disregard the temporal and spatial structure essential for robotic tasks under closed-loop control."* The earlier attribution to "VLAs' short action sequences" is not in the paper and was removed. Table 2 shows FastV at unchanged FLOPs (1.864 T) and higher latency (53.28 vs 51.91 ms). |
| 42 | VLA-Pruner reproduces the loss on the same OpenVLA setting and attributes it to a prefill versus action-decode mismatch | **PDF read 2026-09-04**, abstract: *"directly applying VLM-oriented pruning methods to VLA inference can cause severe degradation ... Our analysis attributes this degradation to a key mismatch: VLA inference exhibits distinct attention patterns between the vision-language prefill stage and the action-decode stage"*; *"All analyses are conducted on OpenVLA with the LIBERO benchmark"*. The earlier "reproduces on another setting" was wrong: same OpenVLA / LIBERO setting as VLA-Cache Table 2. |
| 43 | VLA-Pruner runs OpenVLA on LIBERO and SIMPLER, OpenVLA-OFT and pi0 on LIBERO only | **PDF read 2026-09-04**: Table 1 (OpenVLA, OpenVLA-OFT on LIBERO), Table 2 (SIMPLER, OpenVLA), Table 4 (pi0 on LIBERO), plus a real xArm6. So one backbone is crossed with two benchmarks and the others are not. The earlier "do not report the crossing cell" was wrong for this paper and the sentence now says "where one backbone does appear on two benchmarks the others appear on one". |
| 44 | SpecPrune-VLA classifies actions as coarse or fine by end-effector speed and prunes more in coarse phases; failures cluster at contact | **PDF read 2026-09-04**, abstract: *"We classify actions as coarse- or fine-grained by the speed of the end effector and adjust pruning aggressiveness accordingly"*; Sec. 6: *"failures predominantly occurred during object-contact phases, such as manipulation or placement"*. Grid: LIBERO with OpenVLA-OFT, SimplerEnv with DB-OFT (Table 1). |
| 45 | VLA-IAP, EfficientVLA, MoLe-VLA, Look Focus Act, Traver, StarVLA | **PDFs read 2026-09-04.** VLA-IAP: *"Interaction-Aligned Pruning ... training-free"*, Table 1 evaluates *"DreamVLA, pi0 (LIBERO) and pi0.5 (VLABench)"*. EfficientVLA: *"training-free"*, *"Importance-Driven Non-Contiguous Layer Pruning"* with a *"similarity-derived importance metric"*. MoLe-VLA: *"Spatial-Temporal Aware Router (STAR)"* and *"self-knowledge distillation (CogKD)"*. Look Focus Act: *"open-source pretrained ViT weights ... cannot be directly applied as pretraining is done on fixed, uniform tokenization patterns. To address this, we pretrain our own ViT-B models from scratch"*; foveated tokenization *"significantly reduces the number of tokens"*. Traver and Bernardino: *"A review of log-polar imaging for visual perception in robotics"*, Robotics and Autonomous Systems 2010. StarVLA (arXiv 2604.05014, the codebase paper): *"VLA methods remain fragmented across incompatible architectures, codebases, and evaluation protocols"*. |
| 46 | Lineage and existence citations, checked 2026-09-04 | **PDFs read.** effvlasurvey1: *"Efficient Vision-Language-Action Models for Embodied Manipulation: A Systematic Survey"* (Guan et al.). effvlasurvey2: *"A Survey on Efficient Vision-Language-Action Models"* (Yu et al.). dqn (Mnih et al., Nature 2015): *"we also use a simple frame-skipping technique. More precisely, the agent sees and selects actions on every kth frame instead of every frame, and its last action is repeated on skipped frames"*, which is exactly the lineage the axis-1 sentence claims. openvlaoft: OFT recipe *"integrates parallel decoding, action chunking, a continuous action representation"*. bagoftricks_cnn: *"most refinements are either briefly mentioned as implementation details or only visible in source code. In this paper, we will examine a collection of such refinements"*, the sentence the RW claim paraphrases. bagoftricks_llm: *"Bag of Tricks for Inference-time Computation of LLM Reasoning"*, cited as genre precedent only. fastv, sparsevlm, tome: titles confirmed as VLM / ViT token methods (FastV, SparseVLM, Token Merging). NOT read, no PDF in the uploads folder, cited for existence only: act, diffusionpolicy, schwartz, simplerenv, libero. |
| 47 | act, diffusionpolicy, simplerenv, libero, schwartz | **PDFs read 2026-09-04** (ACT, Diffusion Policy, SimplerEnv uploaded by the author; LIBERO text pasted by the author). ACT: *"we take inspiration from action chunking ... the policy predicts the target joint positions for the next k timesteps"*, the origin of the term. Diffusion Policy: *"receding horizon control"*, *"accommodates high-dimensional action sequences"*. SimplerEnv: title *"Evaluating Real-World Robot Manipulation Policies in Simulation"*, *"two commonly used robot embodiments (Google Robot and WidowX)"*. LIBERO: *"four task suites (130 tasks in total)"*, matching "the four suites of LIBERO". Schwartz: bib moved from the 1980 Vision Research paper to the 1977 Biological Cybernetics paper, which Traver and Bernardino list first ([10]) as the origin of the mapping; the uploaded scan (schwartz1977spatial.pdf) has no text layer, so pages 1 and 2 were rendered and read as images: Biol. Cybernetics 25, 181--194 (1977), abstract *"The retinotopic mapping of the visual field to the surface of the striate cortex is characterized as a logarithmic conformal mapping"*, and *"a complex logarithmic (conformal) mapping of the visual field onto the cortical surface"*, which is the log-polar mapping the Related Work sentence names. The arXiv ids in the act, diffusionpolicy and libero notes were added from memory because arXiv is blocked here; the conference PDFs carry no stamp. Verify before camera-ready. |
