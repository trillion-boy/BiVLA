...
class PutOnInSceneEnv(MoveNearInSceneEnv):
    ...
    def evaluate(self, success_require_src_completely_on_target=True, z_flag_required_offset=0.02, **kwargs):
        ...
        ever_grasped_src = self.episode_stats["is_src_obj_grasped"] or is_src_obj_grasped
        success = (
            src_on_target
            and moved_correct_obj
            and (not moved_wrong_obj)
            and ever_grasped_src
        )
        ...
        return dict(
            ...
            ever_grasped_src=ever_grasped_src,
...
class PutEggplantInBasketScene(PutOnBridgeInSceneEnv):
    def __init__(
        self,
        legacy_unstable_quats: bool = False,
        **kwargs,
    ):
        ...
        stable_quat_configs = [
            ...
            np.array([
                euler2quat(0, 0, np.pi / 4, "sxyz"),
                [1, 0, 0, 0],
            ]),
            # Keep 24 episode ids by duplicating a stable yaw instead of the
            # legacy -45 deg bucket, which is the one that produces contact
            # discontinuities for the eggplant in the sink scene.
            np.array([
                euler2quat(0, 0, 0, "sxyz"),
                [1, 0, 0, 0],
            ]),
        ...
        legacy_quat_configs = [
            stable_quat_configs[0],
            stable_quat_configs[1],
            np.array([
                euler2quat(0, 0, -np.pi / 4, "sxyz"),
                [1, 0, 0, 0],
            ]),
        ]
        quat_configs = legacy_quat_configs if legacy_unstable_quats else stable_quat_configs
